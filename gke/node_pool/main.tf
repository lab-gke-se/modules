resource "google_container_node_pool" "node_pool" {
  ...
  dynamic "node_config" {
    for_each = try(var.config, null) != null ? [var.config] : []
  ...
  dynamic "network_config" {
    for_each = try(var.networkConfig, null) != null ? [var.networkConfig] : []
    ...
    content {
      ...
      pod_ipv4_cidr_block  = try(network_config.value.podIpv4CidrBlock, null)
  ...
  max_pods_per_node = try(var.maxPodsConstraint.maxPodsPerNode, null)

  dynamic "upgrade_settings" {
    for_each = try(var.upgradeSettings, null) != null ? [var.upgradeSettings] : []
  ...
  dynamic "queued_provisioning" {
    for_each = try(var.queuedProvisioning, null) != null ? [var.queuedProvisioning] : []
